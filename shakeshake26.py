...
class ResNet32x32(nn.Module):
    def __init__(self, block, layers, channels, groups=1, low_dim=64, num_classes=1000, downsample='basic', proj=True):
        super().__init__()
        assert len(layers) == 3
        self.downsample_mode = downsample
        self.inplanes = 16
        ...
        if proj:
            ...
            self.fc1 = nn.Linear(block.out_channels(channels * 4, groups), block.out_channels(channels * 4, groups))
            self.relu_mlp = nn.LeakyReLU(inplace=True, negative_slope=0.1)
            self.fc2 = nn.Linear(block.out_channels(channels * 4, groups), low_dim)
